...
%
\Title{Low-lying Zeros of Elliptic Curve $L$-functions}
\Author{Simon Spicer}
\Year{2015}
...
\setcounter{page}{-1}
\abstract{%
Something something elliptic curves!
}
...
% ----- dedication
%
\dedication{\begin{center}To my wife Kimberly and son Bram, who comprise two thirds of the Spicer Theorem.\end{center}}
...
\chapter{An Algorithm to Compute Rank}
 
\section{Proof of the Main Theorem}\label{sec:main_thrm_proof}

In this section we prove Theorem \ref{thm:main_theorem}: specifically, that Algorithm \ref{algo:compute_rank} is guaranteed, assuming BSD and ABC, to correctly output an elliptic curve's rank in time $\softO(\sqrt{N_E})$, where $N_E$ is the conductor of the input curve. Note that the proof will quote certain results established later in this work. \\
...
\begin{equation}
k = 4 + 10.5 \log_2 N_E - \log_2 \Omega_E
\end{equation}
...
\end{equation}
Now by Theorem \ref{thm:regulator_lower_bound} we have that $\Reg_E\ge 4429.16 N_E^{-10.49}$. Thus 
$\log_2 \Reg_E \ge 12.11 - 10.49 \log_2 N_E$. We therefore have that
\begin{equation}
\log_2 C_E\pr \ge -3.89 - 10.49 \log_2 N_E + \log_2 \Omega_E > -k
\end{equation}
...
\end{proof}
In other words, the leading Taylor coefficient of $\Les$ at $s=1$ must be greater than $2^{-k}$ in magnitude. \\

We now prove Theorem \ref{thm:main_theorem}: that, assuming BSD, GRH and ABC, Algorithm \ref{algo:compute_rank} computes the rank of an elliptic curve in $\softO\left(\sqrt{N_E}\right)$ time.
\begin{proof}
...
In terms of time complexity, observe that $k$ is $O(\log N_E)$ in magnitude, and by Corollary \ref{cor:real_period_time_complexity}, can be computed in time polynomial in $\log N_E$. By Corollary \ref{cor:better_an_bound}, we need to evaluate at most $\frac{1}{2}\log N_E - 0.4$ central Taylor coefficients of $\Les$ to $k$ bits precision; Proposition \ref{prop:L_E_time_complexity} states that each of these can be done in $\softO\left(k\cdot \sqrt{N_E}\right)$ time. Hence the algorithm is guaranteed to terminate in time at most 
\begin{equation}
O(\log N_E) + \left(\frac{1}{2}\log N_E - 0.4\right) \cdot \softO\left(k\cdot \sqrt{N_E}\right) = \softO\left(\sqrt{N_E}\right)
\end{equation}
since $k$ is sub-polynomial in $N_E$.
\end{proof}
...
\end{align*}
If one assumes GRH, the by Corollary \ref{cor:better_an_bound} we have $r < \frac{1}{2} \log N_E - 0.4$; proceed as above to obtain the second inequality.
\end{proof}
