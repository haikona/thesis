...
%\begin{equation}
%V _L \ge \frac{\sqrt{r+1}}{2^{r/2}} \cdot h^r
%\end{equation}
%\end{lemma}
%\begin{proof}
%Let $L \subset \RR^r$ be a lattice with basis $\set{v_1,\ldots,v_r}$ subject to the constraint that the minimum nonzero vector length in $L$ is $h$. We want to minimize the covolume $V_L$ by varying the basis vectors $v_i$. Let $\en{\cdot}$ be the Euclidean norm on $\RR^r$; observe that to minimize $V_L$ we must have
%\begin{equation*}
%\en{v_1}=\en{v_2}=\cdots=\en{v_r} = h,
%\end{equation*}
%as $V_L$ scales directly with the basis vector lengths. Next, we vary the angles between the basis vectors to minimize covolume, and get
%\begin{equation*}
%\en{v_i-v_j}=h \;\;\text{for all $i\ne j$},
%\end{equation*}
%i.e. every basis vector is at an angle of $\frac{\pi}{3}$ from every other basis vector. This is because increasing the angle between any 2 basis vectors beyond $\frac{\pi}{3}$ increases the covolume, while reducing the angle between any$v_i$ and $v_j$ beyond $\frac{\pi}{3}$ would then have that $\en{v_i-v_j} <h$, violating the minimum vector length constraint. \\
%
%Thus the polytope whose vertices are $\set{v_1,\ldots,v_r}$ along with the zero vector is just the regular $r$-simplex (i.e. the $r$-dimensional generalization of the equilateral triangle/tetrahedron) of side length $h$, whose volume is
%\begin{equation}
%\frac{\sqrt{r+1}}{2^{r/2} \, r!} \cdot h^r
%\end{equation}
%(see for a proof). Finally, note that the volume of a $r$-parallelotope is $r!$ times the volume of any $r$-simplex that shares $r$ converging edges with it. But the lattice covolume $V_L$ is just the volume of the regular $r$-parallelotope with side lengths all equal to $h$ and all interior angles between edges connected to the origin being equal to $\frac{\pi}{3}$. We thus get
%\begin{equation}
%V_L = \frac{\sqrt{r+1}}{2^{r/2}} \cdot h^r
%\end{equation}
%This is therefore the minimum covolume an $r$-dimensional lattice can have such that every nonzero vector has length at least $h$, from which the result follows.
%\end{proof}

Even though the Elkies bound above would seem so small as to be  of limited use in practical applications, we can use it to bound a curve's regulator from below in terms of an inverse power of its conductor. For this we will need the following geometric lemma:

%\begin{lemma}\label{lem:hermite_covolume_bound}
%Let $L$ be a lattice in $\RR^r$ with covolume $V_L$. If $h$ is the minimum nonzero vector length in $L$, then
%\begin{equation}
%V _L \ge \left(\frac{3}{4}\right)^{\frac{r(r-1)}{2}} h^r.
%\end{equation}
%\end{lemma}
%A proof can be found in Lang's Survey of Diophantine Geometry \cite[Chapter 5, Corollary 7.8]{Lang-1983}. With this we then have the following:
